...
export class FunctionRegistry {
  ...
  execute(name: string, args: Record<string, unknown>): unknown {
    ...
    try {
      ...
      if (cause instanceof Error && "diagnostic" in cause) {
        throw cause;
      }
      const error = diagnostic(
...
export function getFunctionDiagnostic(error: unknown): Diagnostic | null {
  if (
    error &&
    typeof error === "object" &&
    "diagnostic" in error &&
    ...
    return (error as { diagnostic: Diagnostic }).diagnostic;
  }
