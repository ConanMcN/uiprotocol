...
import type { ParseResult } from "@uiprotocol/core";
...
function parseJson(raw: string): ParseResult<unknown> {
  try {
    ...
    return parseFailure([
      diagnostic(
        A2UI_DIAGNOSTIC_CODES.invalidJson,
...
function validateUpdateComponents(
  ...
  if (!surfaceId) {
    return parseFailure([
      diagnostic(
        A2UI_DIAGNOSTIC_CODES.missingField,
  ...
  if (!Array.isArray(payload.components)) {
    return parseFailure([
      diagnostic(
        A2UI_DIAGNOSTIC_CODES.invalidType,
  ...
  for (let index = 0; index < payload.components.length; index += 1) {
    ...
    if (childIds.includes(patch.id as string)) {
      warnings.push(
        diagnostic(
          A2UI_DIAGNOSTIC_CODES.invalidEnvelope,
          `Component '${patch.id}' references itself as a child.`,
          { severity: "warning", nodeId: patch.id as string }
        )
      );
    }
  }
  ...
  return parseSuccess(
    payload as unknown as UpdateComponentsPayload,
    warnings
  );
}
...
export function parseMessage(raw: unknown): ParseResult<A2UIMessage> {
  ...
  if (!initial.ok) {
    return initial as ParseResult<A2UIMessage>;
  }
  ...
  if (value.version !== "v0.9") {
    return parseFailure([
      diagnostic(
        A2UI_DIAGNOSTIC_CODES.unsupportedVersion,
  ...
  switch (messageType) {
    ...
    case "updateComponents": {
      ...
      if (!validated.ok) {
        return validated as ParseResult<A2UIMessage>;
      }
      return parseSuccess(
        value as unknown as A2UIMessage,
        validated.warnings
      );
    }
    ...
    default:
      return parseFailure([
        diagnostic(
          A2UI_DIAGNOSTIC_CODES.unknownMessageType,
